...
도메인 전환은 `GameSession` 메서드 한 곳에서만 발생한다.
...
`Court`는 더 이상 app-visible domain phase가 아니다. court simulation은 `complete_training_action` 내부에서 즉시 끝나고, app은 생성된 로그를 `CourtReplay` 화면에서 재생한다.
